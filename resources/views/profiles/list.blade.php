...
<html lang="en">
  ...
  <body>
              ...
              <div class="card border-0 shadow lg my-4">
                ...
                <!-- Search Form  -->
                 <div class='card-header'>
                    <input type="search" id="search" name="search" class="search form-control" placeholder="Search" aria-label="Search">
                 </div>

                 {{-- Partial Table Entry --}}
                 <div id="table-data">

                    @include('profiles.partialTable')
    ...
    //                             +editRoute+'" class="btn btn-sm"><i class="bi bi-pencil-square h4"></i></a><button  type="button" data-id=" '
    //                             +item.id + ' " class="deleteButton btn btn-sm text-danger"><i class="bi bi-trash h4"></i></button></div></td></tr>');
    //                         });
    ...
    //                                 if (link.url) {
    //                                     $('#pagination_search').append('<button class="page-link" data-page="' + (new URL(link.url).searchParams.get('page') || 1) + '">' + link.label + '</button>');
    //                                 } else {
    //                                     $('#pagination_search').append('<span>' + link.label + '</span>');
    //                                 }
    //                             });
    //                         }
    ...
    //         fetchResults();
    //          // Initial profile list load
    //         //  attachProfileListPaginationHandler();
    // });
    ...
    $(document).ready(function(){
        ...
        $(document).on('click', '.deleteButton', function() {
            var id = $(this).data('id');
            // alert(id);
            var url = '{{ route("profiles.destroy", ":id") }}'.replace(':id', id);
